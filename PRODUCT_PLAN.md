...
- [ ] DB 스키마 v1 작성
- [ ] 와이어프레임 5개 화면 제작
<<<<<<< codex/create-webtoon-community-site-vyqek8

## 10) 지금 바로 할 일 (오늘~이번 주)
### 오늘 (Day 1)
- [ ] 서비스 이름 3개 후보 정하고 도메인 가능 여부 확인
- [ ] MVP 범위 확정: 작품 커뮤니티 + 캐릭터 커뮤니티 + 스포일러 태그까지만
- [ ] 피그마에 핵심 화면 3개(홈/작품상세/글상세) 저해상도 와이어프레임 작성

### 이번 주 (Week 1)
- [ ] 저장소 구조 확정 (frontend/backend 분리 여부)
- [ ] PostgreSQL 기준 ERD 작성 및 마이그레이션 초안 생성
- [ ] 게시글/댓글 API 명세서(OpenAPI or Notion) 작성
- [ ] 로그인 없이 읽기 가능, 로그인 후 쓰기 가능 정책 확정

### 다음 주 (Week 2)
- [ ] 작품 상세 + 작품 게시판 CRUD 구현
- [ ] 캐릭터 상세 + 캐릭터 게시판 CRUD 구현
- [ ] 스포일러 플래그/가림 UI 구현

## 11) 첫 출시 기준 (MVP Done 정의)
- 사용자는 작품 페이지에서 글 작성/조회/댓글 가능
- 사용자는 캐릭터 페이지에서 글 작성/조회/댓글 가능
- 스포일러 체크된 글은 기본 가림 처리
- 운영자는 신고된 글을 숨김 처리 가능
- 최소 30개 작품, 작품당 대표 캐릭터 2명 이상 데이터 등록
=======
>>>>>>> main
